...
### Phase 6 — Polish — **✅ verified 2026-07-24** (incl. configurable dictation shortcut)
...
Spec: `PRD - Native macOS Dictation & Transcription utility.md`, which since 2026-08-01 carries the history & audio retention requirements too (features 5–9) — the separate `PRD - Transcription History & Audio Retention.md` was folded into it and removed. Branch `feature/history-and-audio-retention`. Milestones M1–M5 below are the plan's own; their scope is stated here.

- **M1 — history store** — ✅ verified. `HistoryEntry` (SwiftData, versioned schema v1) + `HistoryStore` + `AppDirectories`; every completed dictation is archived with its transcript, timeline segments, locale, duration, target app and delivery outcome; Settings gains a History tab with the on/off toggle. Audio and pin fields are in the v1 schema but unused, so M3/M4 need no migration. `finishSession()` now returns a `SessionResult` instead of a `String`, and `transcriptionIncomplete` carries one too, so truncated sessions are archived rather than only inserted.
- **M2 — History window** — ✅ verified. Searchable, date-grouped list + detail pane with Copy / Insert Again / Save… / Delete; multi-select; menu bar History… (⌘Y), Recent Transcripts submenu, Copy Last Transcript. Re-insertion goes through `FrontmostAppTracker` + `OutputRouter.reinsert()`, which re-activates the last non-Transcriber app and waits for the activation before pasting — the live-dictation path can't be reused because the History window and the menu bar both take focus.
- **M3 — audio retention + playback** — ✅ verified. `SessionAudioRecorder` tees the already-converted analyzer buffers into an AAC `.m4a` on a serial queue (never the render thread); opt-in, off by default; playback with scrubber and 1×/1.5×/2× in the detail pane, plus Save Audio / Reveal / Delete Audio. Files are **timestamp-named** (`2026-07-30 10-45-12.m4a`) so the folder is browsable, reserved with `O_EXCL` so same-second recordings can't collide.
- **M4 — retention policy, pruning, pin UI, export** — ✅ verified. Age expiry + audio size cap as pure functions in `HistoryPruningPolicy` (both skip pinned entries); pruning at launch and after each recorded dictation; launch reconciliation against the recordings folder; storage readout, Export (Markdown + `history.json`), Delete All; Pause History in the menu; audio retention skipped while secure input is active.
- **M5 — re-transcribe** — ✅ verified. Sheet with locale picker and Replace / Save-as-new, running the existing `FileTranscriber` over the retained `.m4a` under the same busy gating as a dropped file. A new entry gets its own copy of the audio so deleting either never strands the other. Segment highlighting during playback landed; per-word click-to-seek did not (see PRD F7.13).

**Verified:** the full Phase 8 manual pass in the PRD's §14 list. The M1 store check — dictate 3×, quit, relaunch, confirm the log reports the accumulated count:

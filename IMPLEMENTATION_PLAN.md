...
- **M3 — audio retention + playback** — ✅ built, ⏳ awaiting manual check. `SessionAudioRecorder` tees the already-converted analyzer buffers into an AAC `.m4a` on a serial queue (never the render thread); opt-in, off by default; playback with scrubber and 1×/1.5×/2× in the detail pane, plus Save Audio / Reveal / Delete Audio. Files are **timestamp-named** (`2026-07-30 10-45-12.m4a`) so the folder is browsable, reserved with `O_EXCL` so same-second recordings can't collide.
- **M4 — retention policy, pruning, pin UI, export** — ✅ built, ⏳ awaiting manual check. Age expiry + audio size cap as pure functions in `HistoryPruningPolicy` (both skip pinned entries); pruning at launch and after each recorded dictation; launch reconciliation against the recordings folder; storage readout, Export (Markdown + `history.json`), Delete All; Pause History in the menu; audio retention skipped while secure input is active.
- **M5 — re-transcribe** — ✅ built, ⏳ awaiting manual check. Sheet with locale picker and Replace / Save-as-new, running the existing `FileTranscriber` over the retained `.m4a` under the same busy gating as a dropped file. A new entry gets its own copy of the audio so deleting either never strands the other. Segment highlighting during playback landed; per-word click-to-seek did not (see PRD §14).
